...
def inicializar_base_datos():
    ...
    if not os.path.exists(DB_FILE):
        ...
        # Si el archivo existe pero es viejo, le inyectamos las columnas que le falten
        try:
            df_existente = pd.read_csv(DB_FILE)
            cambios = False
            for col in columnas_requeridas:
                if col not in df_existente.columns:
                    df_existente[col] = ""  # Agrega la columna vacía
                    cambios = True
            if cambios:
...
if df_render.empty:
    ...
    for idx, row in df_render.iterrows():
        ...
        # Botonera de interacción abajo de la tarjeta corregida con identificadores únicos
        col_web, col_wpp, col_ig, col_map = st.columns([2, 1, 1, 1])
        
        with col_web:
            st.link_button("🌐 Visitar Sitio", row['Subdominio'], use_container_width=True, key=f"web_{idx}")
        ...
        with col_wpp:
            if row['WhatsApp']:
                ...
                st.link_button("🟢 Wpp", f"https://wa.me/{clean_wpp}", use_container_width=True, key=f"wpp_act_{idx}")
            else:
                st.button("🟢", disabled=True, use_container_width=True, help="No tiene WhatsApp", key=f"wpp_dis_{idx}")
                
        with col_ig:
            if row['Instagram']:
                clean_ig = str(row['Instagram']).replace("@", "").strip()
                st.link_button("📸 IG", f"https://instagram.com/{clean_ig}", use_container_width=True, key=f"ig_act_{idx}")
            else:
                st.button("📸", disabled=True, use_container_width=True, help="No tiene Instagram", key=f"ig_dis_{idx}")
                
        with col_map:
            if row['GoogleMapsRaw']:
                url_final_mapa = row['GoogleMapsRaw'] if "http" in str(row['GoogleMapsRaw']) else f"https://www.google.com/maps?q={row['GoogleMapsRaw']}"
                st.link_button("📍 Mapa", url_final_mapa, use_container_width=True, key=f"map_act_{idx}")
            else:
                st.button("📍", disabled=True, use_container_width=True, help="No tiene mapa", key=f"map_dis_{idx}")